...
template<typename T, typename F = function<T(const T&, const T&)>> class segtree {
  public:
      ...
      if (L == neutral && R != neutral) return R;
      else if (R == neutral && L != neutral) return L;
      else if (L == R && L == neutral) assert(false);
			return f(L, R);
		}
...
int32_t main() {
  ...
  int N, Q; cin >> N >> Q;
  vector<Mint> A(N); for (auto &x : A) cin >> x;
  segtree<Mint> st(N, A, 0, [&](Mint a, Mint b) {
    long long aa = (long long) a;
    long long bb = (long long) b;
    return (Mint) gcd(aa, bb);
  });
  while (Q--) {
    int i, x; cin >> i >> x, i--;
    st.update(i, x);
